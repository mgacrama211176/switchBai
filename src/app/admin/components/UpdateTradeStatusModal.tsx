...
export default function UpdateTradeStatusModal({
  trade,
  onClose,
  onSuccess,
}: UpdateTradeStatusModalProps) {
  const [selectedStatus, setSelectedStatus] = useState(trade.status);
  ...
  const statusOptions = [
    {
      value: "pending",
      label: "Pending",
      description: "New trade awaiting confirmation",
    },
    {
      value: "confirmed",
      label: "Confirmed",
      description: "Trade confirmed and ready to process",
    },
    {
      value: "completed",
      label: "Completed",
      description: "Trade completed (inventory will be updated)",
    },
    {
      value: "cancelled",
      label: "Cancelled",
  ...
  const availableStatuses = validTransitions[trade.status] || [];
  ...
  async function handleUpdate() {
    if (selectedStatus === trade.status) {
    ...
    try {
      const response = await fetch(`/api/trades/${trade._id}/status`, {
        method: "PATCH",
        headers: {
      ...
      <div className="bg-white rounded-2xl max-w-md w-full">
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200">
          <div>
            ...
            <p className="text-gray-600">Trade: {trade.tradeReference}</p>
          </div>
          <button
            onClick={onClose}
          ...
          <div>
            ...
            >
              {statusOptions.map((option) => {
                ...
                const isCurrent = option.value === trade.status;
                return (
                  <option
                    key={option.value}
                    value={option.value}
          ...
          {availableStatuses.length === 0 && (
            <div className="bg-gray-50 border border-gray-200 rounded-lg p-4">
              <p className="text-sm text-gray-600">
                This trade is in a terminal state and cannot be updated.
              </p>
            </div>
          )}
        </div>
